//! Raise open questions for a doc + helpers for question-pool gathering and
//! candidate filtering.
...
mod tests {
	...
	async fn dedupe_runs_after_template_filter() {
		...
		let title = "What causes X?";
		let hash = fnv_question_id(title);
		let tags = vec!["question".to_string(), "p1".to_string(), hash];
		let q = store::create_document(root, "questions", title, "body", tags, Some("p1"), None).unwrap();
		cache::pool_insert(root, cache::PoolEntry {
			doc_type: "questions".to_string(),
			doc: store::Document {
				id: q.id.clone(),
		...
		assert_eq!(keep_idx, vec![1]);

		assert_eq!(templated_dropped, 1);
		assert_eq!(semantic_dropped, 1);
		assert_eq!(keep_idx.len(), 1);

		cache::pool_remove(root, &q.id);
	}
}
