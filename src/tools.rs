...
impl WikiService {
	...
	async fn learn_pass(&self, params: Parameters<LearnPassParams>) -> String {
		let LearnPassParams { limit, purpose, dry_run, qa, force } = params.0;
		let limit = limit.map(|n| n as usize).unwrap_or(25);
		match learn::run_pass(self.root(), limit, purpose.as_deref(), dry_run.unwrap_or(false), qa.unwrap_or(true), force.unwrap_or(false)).await {
			Ok(v) => v.to_string(),
			Err(e) => json_err(e),
		}
	}
